...
async function setupCamera() {
    ...
    // La cámara trasera graba en 1080p o 4K. Si pasas eso a la IA, el teléfono explota.
    // Teachable Machine realmente solo necesita ver 224x224 píxeles.
    const maxSize = 300; 
    ...
    // 6. Ocultar placeholder si existe
    // (Ya no es necesario borrar el HTML porque el video está permanentemente ahí)

    // 7. Iniciar loop — dibuja cada frame en el canvas para que el modelo pueda leerlo
    function loop() {
        if (!stream) return;
...
// ─────────────────────────────────────────────
// Cargar datos de rutas y poblar el select
// ─────────────────────────────────────────────
async function loadRouteData() {
    console.log("Cargando datos_rutas.json desde Flask...");
    try {
